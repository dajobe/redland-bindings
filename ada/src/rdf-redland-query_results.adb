...
package body RDF.Redland.Query_Results is
   ...
   type Node_Handle_ptr is access Node_Handle with Convention=>C;
   type Node_Handle_ptr_ptr is access Node_Handle_ptr with Convention=>C;

   function librdf_query_results_get_bindings (Results: Query_Results_Handle;
                                               Names: chars_ptr_ptr_ptr;
                                               Values: Node_Handle_ptr_ptr)
                                               return int
     with Import, Convention=>C;
