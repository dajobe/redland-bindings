...
package RDF.Raptor.IOStream is
   ...
   -- FIXME: Make a tree of classes with separate "builtin" and "user defined" branches.
   ...
   function Read_Eof (Stream: Base_IOStream_Type) return Boolean;

   function Tell (Stream: Base_IOStream_Type) return unsigned_long;

   procedure Write (Value: String; Stream: Base_IOStream_Type);

   procedure Decimal_Write (Value: int; Stream: Base_IOStream_Type);

   procedure Write (Value: char; Stream: Base_IOStream_Type);

   function Write_Bytes (Ptr: chars_ptr; size, nmemb: size_t; Stream: Base_IOStream_Type) return int;

   procedure Write_End (Stream: Base_IOStream_Type);

   procedure Bnodeid_Ntriples_Write (bnode: String; Stream: Base_IOStream_Type);
   ...
   procedure Ntriples_Write (Value: String; Delim: Character; Stream: Base_IOStream_Type);
   ...
   type IOStream_Type_Without_Finalize is new Base_IOStream_Type with null record;

   function From_Sink (World: Raptor_World_Type_Without_Finalize'Class) return IOStream_Type_Without_Finalize;
   ...
   function To_Sink (World: Raptor_World_Type_Without_Finalize'Class) return IOStream_Type_Without_Finalize;
   ...
   --overriding function Default_Handle(Object: User_Defined_IOStream_Type) return Handle_Type;
   not overriding function Open (World: Raptor_World_Type_Without_Finalize'Class)
                                 return User_Defined_IOStream_Type;

   -- We can do initizization and finalization on Ada level.
   -- No need to provide such callbacks to the underlying C library

   --overriding procedure Initialize(Object: in out User_Defined_IOStream_Type);

   --overriding procedure Finalize(Object: in out User_Defined_IOStream_Type);

   not overriding procedure Do_Write_Byte (Stream: in out User_Defined_IOStream_Type; Byte: char);

   not overriding function Do_Write_Bytes (Stream: in out User_Defined_IOStream_Type;
                                           Data: chars_ptr;
                                           Size, Count: size_t)
                                           return int;

   not overriding procedure Do_Write_End (Stream: in out User_Defined_IOStream_Type);

   not overriding function Do_Read_Bytes (Stream: in out User_Defined_IOStream_Type;
                                          Data: chars_ptr;
                                          Size, Count: size_t) return size_t;

   not overriding function Do_Read_Eof (Stream: in out User_Defined_IOStream_Type) return Boolean;

   type Stream_From_String(<>) is new Base_IOStream_Type with private;
   ...
   type Stream_To_String is new Base_IOStream_Type with private;
...
private
   ...
   type Stream_From_String(Length: size_t) is new Base_IOStream_Type with
      record
         Str: char_array(1..Length);
      end record;

   type User_Defined_IOStream_Type is new Base_IOStream_Type with null record;

   type Stream_To_String is new User_Defined_IOStream_Type with
      record
